import { sendLeadSignalEmail } from '../../api/_email.js';
import { createOrUpdateDeal, findOrCreateContact } from '../../api/_hubspot.js';
...
export const onRequest: PagesFunction = async ({ request }) => {
  ...
  const isDev = (typeof process !== 'undefined' ? process.env.NODE_ENV : 'production') !== 'production';
  ...
  if (request.method !== 'POST') {
    return fail(405, 'METHOD_NOT_ALLOWED', 'Unsupported request method.', 'Method Not Allowed');
  }
  ...
  try {
    ...
    hubspot = await createOrUpdateDeal({
      dealId: body.deal?.dealId,
      quoteRef: body.deal?.quoteRef,
      properties,
      contactId,
