...
The following five documents are ChatGPT Project attachments, not guaranteed repository files:
...
Use repository copies only if they exist locally. Do not stop solely because these ChatGPT Project attachments are unavailable to local Codex.

Use the merged GOVAUTH001 audit as the primary repository evidence. Preserve the completed Project KB reconciliation findings already encoded in this work order. Do not invent, reinterpret, or reconstruct missing Project KB content.
