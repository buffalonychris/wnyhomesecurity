...
GOV009-CODEX-CONTEXT-OPTIMIZATION addendum: `docs/governance/CODEX_CONTEXT_OPTIMIZATION_STANDARD_REV01.md` was added after this manifest scan and should be included in the next full manifest regeneration. It is a governance standard for ChatGPT GitHub connector usage, implementation authority boundaries, copy-friendly command formatting, Codex work-order prompt delivery, prompt reuse, future source-map-based prompt minimization, `mainsync.bat` sync guidance, and mandatory Codex Context Usage Reports. It does not authorize source, CSS, route, UI, runtime, API, HubSpot, Stripe/payment, Resend, Gmail/Workspace, scheduling, Cloudflare, secrets, assets, dependencies, package-lock, quote-system behavior, implementation, pricing, claims, or protected-system changes by itself.
