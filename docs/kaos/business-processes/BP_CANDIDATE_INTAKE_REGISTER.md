...
| WNYHS-BP001E | Package / Offer / BOM / Hardware Candidate Extract | Pilot reviewed / Revision requested | Operator Revision Requested | Operator-provided external candidate extraction artifact | Package / offer / BOM / hardware | Not repository authority; no active KAOS authority | Yes | Not approved | KAOS-BP004 reviewed BOM Parts Qualification only in `BP_REVIEW_KAOS_BP004_BOM_PARTS_QUALIFICATION_REV01.md`; exact source candidate content is missing; no process approved; no Active KAOS Rule created. |
