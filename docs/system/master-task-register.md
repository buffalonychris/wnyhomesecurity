...
- **Forbidden Scope:** No source code, runtime files, website pages, routes, CSS/tokens, catalog imports, SEO metadata, sitemap, robots.txt, Playwright config, Playwright tests, screenshots, visual baselines, metadata tasks, hooks, Sites configuration, dependencies, package-lock, HubSpot/CRM files, Stripe/payment files, Resend/email files, Cloudflare config, environment files, secrets, automation, production QA, activation or completion of future backlog tasks, merge, or version bump.
- **Target Files:** `.gitignore`, `docs/system/master-task-register.md`.
- **Runtime Systems Affected:** None. Dev QA ignore-rule hygiene only.
- **Documentation Updates Required:** Add this bounded task-register record and mark DONE after validation.
- **Validation Required:** `git status --short`; `git diff --stat`; `git diff --name-only`; `git diff --check`; `npm run build`; `npm run qa:site -- --project=chromium`; confirm `git status --short` does not show `test-results/`, `playwright-report/`, or `blob-report/` after QA.
- **Exit Criteria:** `.gitignore` ignores `test-results/`, `playwright-report/`, and `blob-report/`; this register contains `PLAY005`; only `.gitignore` and this register change; no source/runtime/customer-facing files, Playwright config/tests, screenshots, visual baselines, hooks, Sites configuration, protected systems, dependencies, package-lock, future backlog task completions, or version files change; validation passes or any pre-existing QA/tooling blocker is reported separately; draft PR is opened without merge.
- **Dependencies:** Prompt-created bounded `PLAY005` work order; PLAY001; PLAY002; SITE001; SITEQA001; CODEX run contract; OPS003; OPS004; OPS005; current governance authority chain.
- **Operator Decision Required:** Resolve the existing Site QA preview-server failure or provide a revised validation instruction, then rerun `npm run qa:site -- --project=chromium` before marking DONE.
- **Blocking Notes:** Added `.gitignore` entries for `test-results/`, `playwright-report/`, and `blob-report/` so Playwright runtime artifacts generated by local Site QA runs do not appear as untracked files or block clean working-tree sync. Precheck found `PLAY001`, `PLAY002`, `SITE001`, and `SITEQA001` marked DONE; `PLAY003` remains BLOCKED in the register due to missing browser binaries; no `PLAY004` record was present. `npm run build` passed. `npm run qa:site -- --project=chromium` ran and generated `test-results/site-qa`, but failed all route checks with `net::ERR_CONNECTION_REFUSED` at `http://localhost:4173`, indicating the existing QA harness could not reach a local preview server. Post-QA `git status --short` showed only `.gitignore` and this register, confirming generated runtime artifacts were ignored. No source/runtime files, website pages, routes, CSS/tokens, SEO files, Playwright config, Playwright tests, screenshots, visual baselines, hooks, Sites configuration, HubSpot, Stripe/payment, Resend/email, Cloudflare config, environment files, secrets, dependencies, package-lock, version files, or future backlog task statuses were changed.
