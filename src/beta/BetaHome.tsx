          ...
          <div id="solutions" className="beta-shell">
            ...
            <div className="beta-pillar-grid">
              {betaSolutionNavigation.map((item, index) => {
                const body = <><span className="beta-pillar__number">0{index + 1}</span><h3>{item.label}</h3><p>{item.description}</p><span className="beta-pillar__action">{item.href ? 'Explore pillar →' : 'Destination in development'}</span></>;
                return item.href ? <Link className="beta-pillar" to={item.href} key={item.label}>{body}</Link> : <article className="beta-pillar beta-pillar--provisional" key={item.label}>{body}</article>;
              })}
            </div>
