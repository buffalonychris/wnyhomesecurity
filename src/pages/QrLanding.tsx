...
const QrLanding = () => {
  ...
  return (<div className="qr-page-shell"><header className="qr-topbar"><a href="/qrlanding" className="qr-brand"><img src="/brand/crest-system/IconizedLogo.png" alt="WNY Home Security" /><span>WNY Home Security</span></a><nav><a href="/packages?vertical=home-security">View Packages</a><a href="/our-work?vertical=home-security">See Our Work</a><a href="tel:+17162010364">Call/Text</a><a href="#qr-estimate-form">Schedule Estimate</a></nav></header><main className="qr-landing">
    ...
    {submitted ? <section className="qr-panel qr-success-card"><h2>Estimate request received.</h2><p>We’ll review your request and confirm next steps by your approved contact methods.</p>{failureRequestId ? <p><strong>Reference:</strong> {failureRequestId}</p> : null}</section> : <section className="qr-panel" id="qr-estimate-form"><h2>Start My Estimate</h2><form onSubmit={onSubmit} noValidate>
      ...
      {apiFailure && <p className="qr-error">{apiFailure}{failureRequestId ? ` Reference ID: ${failureRequestId}.` : ''} If needed, call or text 716-201-0364.</p>}<button type="submit" className="qr-cta" disabled={isSubmitting}>{isSubmitting ? 'Submitting…' : 'Request My Estimate'}</button><p className="qr-sub">Prefer to call or text? <a href="tel:+17162010364">716-201-0364</a><br/>Call or text and we’ll help you get started.</p></form></section>}
