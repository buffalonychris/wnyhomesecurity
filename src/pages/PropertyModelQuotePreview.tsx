...
const statusForEvidenceType = (record: PropertyModelRecord, evidenceType: PropertyModelEvidenceItem['evidenceType']) => {
  ...
  return matches
    .map((item) => {
      ...
      const label = item.label || item.sourceReference || optionLabel(propertyEvidenceTypeOptions, item.evidenceType);
      return `${label} (${status})`;
    })
    .join('; ');
};
...
const describeEvidence = (record: PropertyModelRecord, evidenceRef: string) => {
  ...
  return evidence.label || evidence.sourceReference || optionLabel(propertyEvidenceTypeOptions, evidence.evidenceType);
};
...
const EvidenceList = ({ record }: { record: PropertyModelRecord }) => {
  if (record.evidenceItems.length === 0) return <p>No source evidence references entered yet.</p>;

  return (
    <ul className="quote-print-evidence-list">
      {record.evidenceItems.map((item) => (
        <li key={item.id}>
          <strong>{item.label || optionLabel(propertyEvidenceTypeOptions, item.evidenceType)}</strong>
          <span>{optionLabel(propertyEvidenceTypeOptions, item.evidenceType)}</span>
          <span>{optionLabel(propertyEvidenceOrientationOptions, item.orientationSide)}</span>
          <span>{optionLabel(propertyEvidenceStatusOptions, item.status)}</span>
          {item.sourceReference ? <span>Ref: {item.sourceReference}</span> : null}
...
const SolutionBlock = ({ record, solution }: { record: PropertyModelRecord; solution: PropertyModelSolution }) => {
  const concerns = solution.concernServed
    ? record.customerConcerns.filter((concern) => [concern.id, concern.text, concern.category].includes(solution.concernServed))
    : [];
  const servedLabel = concerns.length > 0 ? concerns.map((concern) => concern.text || concern.category).join('; ') : solution.concernServed;
...
const HardwareRow = ({ record, item }: { record: PropertyModelRecord; item: PropertyModelBomLineItem }) => (
  <tr>
    ...
    <td>{[item.dashboardPrepNote, item.installerNote].filter(Boolean).join(' ') || 'No customer-appropriate notes entered.'}</td>
  </tr>
);
    ...
    return (
      <main className="quote-print-shell">
        <section className="quote-print-page quote-print-empty">
          ...
          <p>Create or save a Property Model draft before opening the customer-facing preview.</p>
          <Link className="btn btn-primary quote-print-no-print" to="/operator/property-model">
      ...
      <section className="quote-print-page">
        <header className="quote-print-header">
          <div>
            ...
            <p>Prepared for {customerLabel}. This browser-print view uses the local Property Model draft only.</p>
          </div>
        ...
        <section className="quote-print-section">
          ...
          <dl className="quote-print-details">
            <div><dt>Property</dt><dd>{address || customerLabel}</dd></div>
            ...
            <div><dt>Floorplan / evidence summary</dt><dd>{record.evidenceItems.length} local evidence reference(s) entered.</dd></div>
            <div><dt>Hand-drawn floorplan</dt><dd>{statusForEvidenceType(record, 'hand_drawn_floorplan')}</dd></div>
            <div><dt>Professional redraw</dt><dd>{statusForEvidenceType(record, 'professional_redraw')}</dd></div>
            <div><dt>Exterior photo evidence</dt><dd>{statusForEvidenceType(record, 'exterior_photo')}</dd></div>
            <div><dt>Interior photo evidence</dt><dd>{statusForEvidenceType(record, 'interior_photo')}</dd></div>
            <div><dt>Orientation / compass</dt><dd>{statusForEvidenceType(record, 'compass_orientation_note')}</dd></div>
          </dl>
          {record.propertyContext.notes ? <p>{record.propertyContext.notes}</p> : null}
          <p className="quote-print-note">Final placement depends on the approved property plan where a floorplan or redraw is part of the job scope.</p>
        ...
        <section className="quote-print-section">
          ...
          <div className="quote-print-card-grid">
            {record.proposedSolutions.length > 0 ? (
              record.proposedSolutions.map((solution) => <SolutionBlock key={solution.id} record={record} solution={solution} />)
            ) : (
        ...
        <section className="quote-print-section">
          ...
          {record.bomLineItems.length > 0 ? (
            <div className="quote-print-table-wrap">
              <table className="quote-print-table">
                <thead>
                  <tr>
                    <th>Hardware / BOM item</th><th>Qty</th><th>Room / area</th><th>Concern served</th><th>Selected solution</th><th>Evidence ref</th><th>Customer-appropriate notes</th>
                  </tr>
                </thead>
          ...
          <h3>Payment terms</h3>
          <ul className="quote-print-terms">{paymentTerms.map((term) => <li key={term}>{term}</li>)}</ul>
