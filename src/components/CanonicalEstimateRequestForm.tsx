...
const CanonicalEstimateRequestForm = ({ entryRoute, sourceFamily, source, landingRoute, requestId, onFirstInteraction, submitEventName, context }: Props) => {
  ...
  return submitted ? <section className="qr-panel qr-success-card"><h2>Estimate request received.</h2><p>We’ll review your request and confirm next steps by your approved contact methods.</p>{failureRequestId ? <p><strong>Reference:</strong> {failureRequestId}</p> : null}</section> : <form onSubmit={onSubmit} noValidate className="estimate-form-shell">
      <fieldset className="qr-section estimate-form-stage"><legend>Stage 1 — Start Here</legend><div className="qr-form-grid">{([['firstName','First name'],['lastName','Last name'],['mobilePhone','Mobile phone'],['email','Email']] as const).map(([f,l]) => <label key={f} className="estimate-field"><span>{l}</span><input ref={(e)=>{fieldRefs.current[f]=e;}} type={f==='email'?'email':f==='mobilePhone'?'tel':'text'} value={formState[f]} onChange={handleChange(f)} required /></label>)}</div><label className="estimate-field"><span>What do you need help with?</span><select value={formState.requestedHelp} onChange={handleChange('requestedHelp')} required><option value="">Select a service</option><option value="Complete Security System">Complete Security System</option><option value="Cameras Only">Cameras Only</option><option value="System Repair">System Repair</option><option value="System Upgrade">System Upgrade</option><option value="Smart Home Automation">Smart Home Automation</option><option value="Not Sure Yet">Not Sure Yet</option></select></label><label className="estimate-field"><span>Additional details / what you want protected</span><textarea value={formState.requestDetails} onChange={handleChange('requestDetails')} /></label></fieldset>
      <fieldset className="qr-section estimate-form-stage"><legend>Stage 2 — Property Details</legend><div className="qr-form-grid">{([['streetAddress','Street address'],['city','City'],['state','State'],['zip','ZIP']] as const).map(([f,l]) => <label key={f} className="estimate-field"><span>{l}</span><input ref={(e)=>{fieldRefs.current[f]=e;}} value={formState[f]} onChange={handleChange(f)} required /></label>)}</div><label className="estimate-field"><span>Property type</span><select value={formState.propertyType} onChange={handleChange('propertyType')} required><option value="">Select property type</option><option value="Home">Home</option><option value="Apartment / Condo">Apartment / Condo</option><option value="Business">Business</option><option value="Other">Other</option></select></label><label className="estimate-field"><span>Where did you hear about us</span><select value={formState.whereDidYouSeeUs} onChange={handleChange('whereDidYouSeeUs')}><option value="">Select one</option><option value="QR Placard">QR Placard</option><option value="Yard Sign">Yard Sign</option><option value="Google">Google</option><option value="Referral">Referral</option><option value="Other">Other</option></select></label></fieldset>
      <fieldset className="qr-section estimate-form-stage"><legend>Stage 3 — Estimate Window</legend><div className="estimate-form-grid-2"><label className="estimate-field"><span>Preferred estimate date</span><input type="date" min={todayIsoDate} value={formState.preferredEstimateDate} onChange={handleChange('preferredEstimateDate')} required /></label><label className="estimate-field"><span>Preferred 1-hour estimate window</span><select value={formState.preferredEstimateTimeSlot} onChange={handleChange('preferredEstimateTimeSlot')} required><option value="">Select a 1-hour window</option>{timeSlotOptions.map((slot)=><option key={slot} value={slot}>{slot}</option>)}</select></label></div></fieldset>
      <fieldset className="qr-section estimate-form-stage"><legend>Stage 4 — Contact Permissions</legend><div className="qr-radio-row">{(['Text','Phone call','Email'] as PreferredContactMethod[]).map((m) => <label key={m} className="qr-choice estimate-choice"><input type="radio" name="preferredContactMethod" value={m} checked={formState.preferredContactMethod===m} onChange={handleChange('preferredContactMethod')} required /><span>{m}</span></label>)}</div><div className="qr-checkbox-list"><label className="qr-choice estimate-choice"><input type="checkbox" checked={formState.textConsent} onChange={handleChange('textConsent')} /><span>Text Messages</span></label><label className="qr-choice estimate-choice"><input type="checkbox" checked={formState.phoneConsent} onChange={handleChange('phoneConsent')} /><span>Phone Calls</span></label><label className="qr-choice estimate-choice"><input type="checkbox" checked={formState.emailConsent} onChange={handleChange('emailConsent')} /><span>Emails</span></label><label className="qr-choice estimate-choice"><input type="checkbox" checked={formState.contactTimeAcknowledgement} onChange={handleChange('contactTimeAcknowledgement')} required /><span>I understand contact hours are 8am–9pm, 7 days/week.</span></label></div></fieldset>
      {apiFailure && <p className="qr-error">{apiFailure}{failureRequestId ? ` Reference ID: ${failureRequestId}.` : ''}</p>}<button type="submit" className="qr-cta estimate-submit" disabled={isSubmitting}>{isSubmitting ? 'Submitting…' : 'Request My Estimate'}</button>
    </form>;
};
